...
#KMS Key for Secrets Manager encryption (only created if atlease one secret is enabled)
...
resource "aws_secretsmanager_secret" "db_credentials" {
  ...
  description = "Database credentials for EKS cluster"
...
resource "aws_secretsmanager_secret" "api_keys" {
  ...
  description = "API keys for EKS cluster"
  kms_key_id  = aws_kms_key.secrets[0].id
...
resource "aws_secretsmanager_secret" "app_config" {
  ...
  description = "Application configuration for EKS cluster"
  kms_key_id  = aws_kms_key.secrets[0].id
  recovery_window_in_days = 7
